...
public class Application {
    ...
    public static void main(String[] args){
        ...
        String usage = "This aplication compare DPBedia and Freebase categories. Indexes created by sk.xjurcak.ir.IndexApplication in INDEX_PATH directory are used for comparison. \n"
                + "Usage: \n"
...
//        boolean create = true;
        ...
        for(Topic topic : topics){
            try {
                Content[] dbPediaTopicCategories = getDBPediaCategoriesLabels(indexDir, topic.getId());
    ...
    private static void compareWithFreebaseTopics(File indexDir, Topic dbPediaTopic, Content[] dbPediaTopicCategories, Topic[] freebaseTopics) throws ParseException {
        ...
        for(Topic topic : freebaseTopics){
            Content[] fCat = getContents(categoriesIndexFile, topic);
            compareWithFreebaseTopic(dbPediaTopic, dbPediaTopicCategories, topic, fCat);
        }
    }

    private static void compareWithFreebaseTopic(Topic dbPediaTopic, Content[] dbPediaTopicCategories, Topic freebaseTopic, Content[] freebaseTopicCategories) {
    ...
    private static Content[] getDBPediaCategoriesLabels(File indexDir, String topicId) throws ParseException {
    ...
    private static Content[] getContents(File categoriesIndexFile, Topic topicType) throws ParseException {
        Content[] result = new Content[topicType.getTypesId().length];

        int i = 0;
        for (String type : topicType.getTypesId()) {
            Content[] categories = CategoriesSearcher.search(categoriesIndexFile, type);
    ...
    private static final void printDbPediaTopic(Topic topic, Content[] types) throws ParseException {
